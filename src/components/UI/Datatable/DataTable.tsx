...
import FixGridToolbarFilterButton from './ButtonsToolbars/FixGridToolbarFilterButton';
    ...
    <GridToolbarContainer sx={dataTableStyles(theme).toolbarTitleWrapper}>
      ...
      {(hasFilter || hasExport) && (
        <Box sx={dataTableStyles(theme).toolbarWrapper}>
          ...
          <Box>
            {hasFilter && <FixGridToolbarFilterButton />}
            {/* Fix caused by MUI update => delete when not needed*/}
            {hasExport && (
              <GridToolbarExport
                csvOptions={{
